...
public class ContainerMacerator extends Container {
    ...
    private TileEntityMacerator m_localTileEntity;
    
    public ContainerMacerator(InventoryPlayer inventoryplayer, TileEntityMacerator tileEntityMacerator) {
    	m_localTileEntity = tileEntityMacerator;
    	
    	
        for (int iRow = 0; iRow < m_iNumSlotRows; iRow++) {
        	for(int iColumn = 0; iColumn < m_iNumSlotColumns; iColumn++) {
        		this.addSlot(new Slot(tileEntityMacerator, 
