...
import net.minecraft.src.vaderetro.entity.tileentity.TileEntityCeramicFurnace;

import java.util.Random;

public class BlockCeramicFurnace extends BlockContainer {

    private final Random random = new Random();
    private final boolean isLit;
    private static boolean keepInventory = false;

    public BlockCeramicFurnace(int id, boolean isLit) {
    ...
    public int idDropped(int id, Random random) {
        return Block.ceramicFurnaceIdle.blockID;
    }
    ...
    public int getBlockTexture(IBlockAccess blockAccess, int x, int y, int z, int sideId) {
        ...
        return sideId != var6 ? 6 : (this.isLit ? this.blockIndexInTexture + 1 : this.blockIndexInTexture);
    }

    @Override
    public int getBlockTextureFromSide(int sideId) {
        return sideId == 3 ? this.blockIndexInTexture : 6;
    }

    @Override
    public boolean blockActivated(World world, int x, int y, int z, EntityPlayer player) {
        if (!world.multiplayerWorld) {
            TileEntityCeramicFurnace tileEntity = (TileEntityCeramicFurnace) world.getBlockTileEntity(x, y, z);
            player.displayGUICeramicFurnace(tileEntity);
        }
        return true;
    }
    ...
    public static void updateBlockState(boolean isActive, World world, int x, int y, int z) {
        ...
        keepInventory = true;
        if(isActive) {
            world.setBlockWithNotify(x, y, z, Block.ceramicFurnaceActive.blockID);
        } else {
            world.setBlockWithNotify(x, y, z, Block.ceramicFurnaceIdle.blockID);
        }
        keepInventory = false;
    ...
    protected TileEntity getBlockEntity() {
        return new TileEntityCeramicFurnace();
    }
