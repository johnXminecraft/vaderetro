...
public class BlockCarrot extends BlockFlower {

    public BlockCarrot(int id, int blockId) {
        super(id, blockId);
    ...
    public void updateTick(World world, int x, int y, int z, Random random) {
        ...
        if(world.getBlockLightValue(x, y + 1, z) >= 9) {
            int blockCarrot = world.getBlockMetadata(x, y, z);
            if(blockCarrot < 7) {
                float growthRate = this.getGrowthRate(world, x, y, z);
                if(random.nextInt((int)(100.0F / growthRate)) == 0) {
                    ++blockCarrot;
                    world.setBlockMetadataWithNotify(x, y, z, blockCarrot);
                }
            }
        }
        ...
        if(!world.multiplayerWorld) {
            for(int var7 = 0; var7 < 7; ++var7) {
                if(world.rand.nextInt(15) <= var5) {
                    ...
                    EntityItem var12 = new EntityItem(world, (double)((float)x + var9), (double)((float)y + var10), (double)((float)z + var11), new ItemStack(Item.carrot));
    ...
    public int idDropped(int var1, Random var2) {
        return var1 == 7 ? Item.carrot.shiftedIndex : -1;
    }
